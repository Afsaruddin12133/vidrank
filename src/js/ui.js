...
class YouTubeStudioUI {
  ...
  attachEventListeners() {
    ...
    chrome.storage.onChanged.addListener((changes, namespace) => {
      if (namespace === 'local' && (changes.usageCount || changes.plan)) {
        this.checkAndApplyPaywall();
      }
    });

    btnRegenerate.addEventListener('click', () => {
      chrome.storage.local.get(["usageCount", "plan", "uid"], (data) => {
  ...
  checkAndApplyPaywall() {
    chrome.storage.local.get(["usageCount", "plan"], (data) => {
      ...
      const limitReached = (data.usageCount || 0) >= 10;
  ...
  updateTagsList(tagsList) {
    ...
    if (hashtagMode && window.TagGenerator?.toHashtags) {
      displayTags = window.TagGenerator.toHashtags(this.tags);
    }
  ...
  copyTagsToClipboard() {
    ...
    chrome.storage.sync.get({ preferredSeparator: "," }, (settings) => {
      ...
      if (hashtagMode && window.TagGenerator?.toHashtags) {
        finalTags = window.TagGenerator.toHashtags(this.tags);
      }
