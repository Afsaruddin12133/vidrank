...
(function () {
  ...
  function showBlinkingPopup(remainingTime, currentUsageCount, uid, usageLimit) {
    const limit = (usageLimit != null && usageLimit >= 0) ? usageLimit : 10;
    ...
    const hitsRemaining = Math.max(0, limit - currentUsageCount);
        ...
        const executeGeneration = async () => {
          ...
          try {
            ...
            if (genResponse && genResponse.success) {
              if (genResponse.description) {
                insertDescriptionToStudio(genResponse.description);
              }

              chrome.storage.sync.get({ maxTagsCount: 35 }, (settings) => {
                let tags = genResponse.tags || [];
              ...
              const errMsg = genResponse?.error || 'Unknown error';
  ...
  async function triggerTagInsertion(tags) {
    ...
    try {
      ...
      let finalTags = tags;
      if (settings.hashtagMode) {
        finalTags = toHashtags(tags);
      }
  ...
  function generateAndInsertAIDescription(callback) {
    ...
    sidebarUI.log("Generating AI description...", "info");
